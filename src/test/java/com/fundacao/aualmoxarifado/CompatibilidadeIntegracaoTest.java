...
class CompatibilidadeIntegracaoTest {
    ...
    void queriesCustomizadas_executamNoH2ComDadosDoSeeder() {
        ...
        assertThat(usuarioRepository.countByPerfilAndAtivoTrue(PerfilUsuario.ADMIN)).isPositive();
    }
    ...
    void telasPrincipais_renderizamSemErroDeTemplate() throws Exception {
        ...
        // Telas próprias desta branch (importador e conta do usuário logado).
        mvc.perform(get("/materiais/importar")).andExpect(status().isOk());
        mvc.perform(get("/minha-conta/senha")).andExpect(status().isOk());
    }
