...
public interface MovimentacaoRepository extends JpaRepository<Movimentacao, Long> {
     ...
     */
    @Query("""
           SELECT new com.fpto.almoxarifado.dto.ConsumoSetorDTO(
                  s.id, s.nome, s.codigoCentroCusto, SUM(m.quantidade))
           FROM Movimentacao m
           JOIN m.setorDestino s
           WHERE (
                   (m.tipo = com.fpto.almoxarifado.domain.TipoMovimentacao.SAIDA
                    AND m.status IN (com.fpto.almoxarifado.domain.StatusMovimentacao.APROVADO,
                                     com.fpto.almoxarifado.domain.StatusMovimentacao.ENTREGUE))
                OR  m.tipo = com.fpto.almoxarifado.domain.TipoMovimentacao.COMPRA_DIRETA
                 )
